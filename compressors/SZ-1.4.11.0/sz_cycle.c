...
void cycle_float(float* iArray, int nx, int ny, int nz, double tolerance, int iterations, float* oArray, int mode)
{
  printf("MODE= %s\n",(mode == 0 ? "ABS" :
                        (mode == 1 ? "REL" : "unknown")));
	int i;
  ...
  {
		stime = clock();
		bytes = SZ_compress_args(SZ_FLOAT, iArray, &outSize, mode, tolerance,tolerance,tolerance,0, r5,r4,r3,r2,r1);
		etime = clock();
  ...
  cTimesAVG = (aetime - astime) / (CLOCKS_PER_SEC * iterations);  
	...
	for (i = 0; i < iterations; ++i)
 	{
		stime = clock();
 		data = SZ_decompress(SZ_FLOAT, bytes, outSize, r5, r4, r3, r2, r1);	
		etime = clock();  
		dTimes[i] = (etime-stime)/CLOCKS_PER_SEC;
